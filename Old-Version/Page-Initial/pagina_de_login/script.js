...
Ao final da inserção de dados escolhida, há o redirecionamento para o ambiente escolhido pelo usuário do site.
*/
...
function validarCliente() {
	...
	if (!nomeRegex.test(nome)) {
		exibirMensagemDOM(
			"O nome do cliente deve ter ao menos 6 caracteres e conter ao menos 1 caractere especial.",
			"erro"
		);
		return false;
	}
	...
	if (!senha || !nome || !email){
	    exibirMensagemDOM("Preencha todos os campos do cliente.", "erro");
	    return false;
	}
...
function validarAdmin() {
	...
	if (!nomeRegex.test(nome)) {
		exibirMensagemDOM(
			"O nome do administrador deve ter ao menos 6 caracteres e conter ao menos 1 caractere especial.",
			"erro"
		);
		return false;
	}
...
window.addEventListener('DOMContentLoaded', () => {
	...
	document.getElementById('form-cliente').addEventListener('submit', (e) => {
		e.preventDefault();
		if (validarCliente()) {
			exibirMensagemDOM("Login do cliente efetuado com sucesso!", "sucesso");
	...
	document.getElementById('form-admin').addEventListener('submit', (e) => {
		e.preventDefault();
		if (validarAdmin()) {
			exibirMensagemDOM("Login do administrador efetuado com sucesso!", "sucesso");
