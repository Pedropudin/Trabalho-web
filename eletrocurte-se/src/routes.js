const ROUTES = {
  APRESENTACAO: '/pages/Apresentacao',
  LOGIN: '/pages/Login',
  PERFIL: '/pages/Perfil',
  EDITAR_PERFIL: '/pages/EditarPerfil',
  RESUMO_COMPRA: '/pages/ResumoCompra',
  CONFIRMA_COMPRA: '/pages/ConfirmacaoCompra',
  DADOS_COMPRA: '/pages/InsercaoDeDadosCompra',
  FINAL_COMPRA: '/pages/CompraFinalizada',
  HIST_COMPRAS: '/pages/HistoricoCompras',
  PAG_PESQUISA: '/pages/PaginaPesquisa',
  PAG_PRODUTO: '/pages/PaginaProduto',
  HIST_PRODUTOS: '/pages/HistoricoProdutos',
  TERMOS: '/pages/TermosCondicoes',
  LOGOUT: '/pages/Logout',
};

export default ROUTES;